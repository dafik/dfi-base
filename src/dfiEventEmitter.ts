...
class EventEmitter {
    ...
    public emit(event: TEventName, a1: any, a2: any, a3: any, a4: any, a5: any, ...args: any[]): boolean {
        ...
        if ("function" === typeof listeners.fn) {
            ...
            for (i = 1, argsS = new Array(len - 1); i < len; i++) {
                argsS[i - 1] = arguments[i];
            }

            listeners.fn.apply(listeners.context, argsS);
        } else {
            ...
            for (i = 0; i < length; i++) {
                ...
                switch (len) {
                    ...
                    default:
                        if (!argsS) {
                            for (j = 1, argsS = new Array(len - 1); j < len; j++) {
                                argsS[j - 1] = arguments[j];
                            }
                        }

                        listeners[i].fn.apply(listeners[i].context, argsS);
                }
            }
        }
